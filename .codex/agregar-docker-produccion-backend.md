...
Se agrego `backend/Dockerfile.prod` para construir una imagen de backend preparada para deploy: instala dependencias, genera Prisma Client, compila TypeScript y arranca la API con `npm start`. Tambien se actualizo `backend/Dockerfile` con el mismo flujo productivo para que Render pueda usar el path `Dockerfile` sin depender de configuracion adicional.
...
Las migraciones Prisma no se ejecutan automaticamente en el `CMD` del contenedor para evitar bloquear el arranque del servicio si `DATABASE_URL` apunta al pooler de Supabase. Cuando haya cambios de schema, `prisma migrate deploy` debe correrse como paso separado usando una connection string directa apta para Prisma Migrate.
